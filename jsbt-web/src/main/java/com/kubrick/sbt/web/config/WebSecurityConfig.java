...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(WebSecurity webSecurity) {
		// 不拦截静态资源,所有用户均可访问的资源
		webSecurity.ignoring().antMatchers("/", "/css/**", "/js/**", "/images/**", "/layui/**", "/v2/api-docs",
				"/doc.html", "/webjars/**", "/swagger-resources/**", "/actuator/**");
	}
	...
	public void configure(HttpSecurity http) throws Exception {
		...
		http.authenticationProvider(authenticationProvider()).httpBasic()
				...
				// .antMatchers(ArrayUtil.toArray(permitAllUrlProperties.getIgnoreUrls(),
				...
				// 登录成功
				.successHandler(customSuccessHandler).and().exceptionHandling()
				// 没有权限
