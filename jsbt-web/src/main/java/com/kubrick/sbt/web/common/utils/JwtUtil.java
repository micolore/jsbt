...
/**
 * @author k JWT校验工具类 iss: jwt签发者; sub: jwt所面向的分组用户; aud: 接收jwt的一方; exp:
 * jwt的过期时间，这个过期时间必须要大于签发时间; nbf: 定义在什么时间之前，该jwt都是不可用的; iat: jwt的签发时间; tid:
...
public class JwtUtil {
	 ...
	 * @return JWT字符串
	 */
	private static String buildJWT(SignatureAlgorithm alg, String sub, String aud, String tid, String iss, Date nbf,
			Integer duration) {
		// jwt的签发时间
		...
		// 获取JWT字符串
		String compact = Jwts.builder().signWith(alg, SECRET) // 加密方式
				.setSubject(sub) // 说明
				.setAudience(aud) // 接收用户
				.setId(tid) // 唯一身份标示
				.setIssuer(iss) // 签发者信息
				.setNotBefore(nbf).setIssuedAt(new Date(iat)) // 签发时间
	...
	/**
	 * 构建JWT 使用 UUID 作为 tid 唯一身份标识 JWT有效时间 1200 秒，即 20 分钟
	 * @param tid jwt 唯一身份标识，主要用来作为一次性token,从而回避重放攻击
	 * @return JWT字符串
	 */
	...
	public static Boolean checkJWT(String token, String sub) {
		try {
			...
			if (claims == null || !claims.getSubject().equals(sub) || claims.getExpiration().before(new Date())) {
				return false;
			}
		}
	...
	public static String generateToken(String userName) {
		...
		String jwt = Jwts.builder().setClaims(map)
				...
				.setIssuedAt(now)
				// 过期时间
				.setExpiration(time).signWith(SECRET).compact();
		// jwt前面一般都会加Bearer
